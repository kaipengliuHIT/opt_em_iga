...
int main(int argc, char *argv[]) {
   ...
   args.AddOption(&lump_only,"-lump-only","--lump-only","Skip Pi_exact (use lumped only).");
   args.Parse();
   ...
   if(!lump_only) BuildPi(*M_op, *Mmix_op, ncurl, no1, PiExact, false);
   BuildPi(*M_op, *Mmix_op, ncurl, no1, PiLumped, true);

   // ── Build Q^T A Q ──────────────────────────────────────────
   DenseMatrix QTAQ_exact, QTAQ_lumped;
   if(!lump_only) BuildQTAQ(*Ah, PiExact, QTAQ_exact);
   ...
   if(!lump_only) for(double w : {0.7,1.0}) {
